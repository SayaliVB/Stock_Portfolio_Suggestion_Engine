...
def get_stocks_and_ratios(strategy, split_equally):
    ...
    return stocks  # We just return the predefined stock ratios without any changes, respecting the predefined ratios
...
def home():
    if request.method == "POST":
        ...
        allocations = get_investment_allocation(investment, strategies, split_equally)
        
        results = []
        total_value = 0

        # Portfolio calculation for each strategy
        for strategy in strategies:
            ...
            portfolio, strategy_total_value = calculate_portfolio_value(
                allocations[strategy], stocks, stock_prices
            )
            results.append({
                "strategy": strategy,
                "portfolio": portfolio,
            ...
            total_value += strategy_total_value  # Accumulate the total portfolio value
