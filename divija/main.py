...
def fetch_stock_prices(stocks):
    ...
    for stock in stocks:
        ...
        prices[stock] = round(info['Close'][-1], 2) if not info.empty else None
    return prices
...
def calculate_portfolio_value(investment, stock_allocations, stock_prices):
    ...
    for stock, ratio in stock_allocations.items():
        ...
        if stock_prices[stock] is None:
            portfolio[stock] = {"allocation": allocation, "price": "N/A", "shares": 0, "graph": None}
        else:
...
def home():
    if request.method == "POST":
        ...
        allocation_per_strategy = investment / len(strategies) if split_equally else investment
    ...
    for symbol in ticker_symbols:
        ...
        if not info.empty:
            price = round(info['Close'][-1], 2)
            change = round(info['Close'][-1] - info['Open'][-1], 2)
